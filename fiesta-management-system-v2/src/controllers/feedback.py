...
class Feedback:
    def view_all_feedbacks(self, grp_id):
        db = Database()
        VIEW_FEEDBACK = ("select c.criteria, f.feedback, f.comments from `groups` as g inner join menu_fdb as f on "
                         "f.menu_id = g.menu_id inner join fdb_criterias as c on f.cr_id = c.id where g.id = %s;")

        response = db.get_items(VIEW_FEEDBACK, (grp_id,))
        return [dict(criteria=tup[0], feedback=tup[1], comments=tup[2]) for tup in response]

    @staticmethod
    def get_menu_fdb_criterias(grp_id):
        db = Database()
        GET_MENU_FDB_CRITERIAS = ("select distinct g.menu_id,r.id,r.criteria from `groups` as g inner join "
...
if __name__ == "__main__":
    feedback = Feedback()
    print(feedback.view_all_feedbacks(1))
