...
class GetMenu(MethodView):
    ...
    def get(self):
        ...
        if len(response) == 0:
            return {
                "error": {
                    "code": 400, "message": "No such menu found."},
                "status": "failure",
                }, 400
        else:
            date = response[0][1]
    ...
    def put(self):
        ...
        if status not in ["pending", "published", "not published", "rejected", "discarded"]:
            abort(400, message='Invalid status, must be from pending, published, not published, rejected, '
                               'discarded')
        else:
            if status == "published":
                ...
                response = menu.reject_menu(menu_id=menu_id, comments=comments, username=username)
                return dict(message=response)
            else:
...
class GetMenu(MethodView):
    ...
    def patch(self, menu_id):
        ...
        menu.update_menu(menu_id=menu_id, old_item=data["old_item"], new_item=data["new_item"])
